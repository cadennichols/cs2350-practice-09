...
import { movies } from './movies' //the dot indicates the file is in the same folder as main.js
...
function searchMovies(event) {
    ...
    let input = document.querySelector('[type="search"]').value || "" //.value get the value from that string
    let count = 0
    for (let m of movies) { //iterate throught the movies
        if (m.title.toUpperCase().indexOf(input.toUpperCase()) == -1) {
            document.querySelector(`#m${m.id}`).classList.add('d-none') //look for the id of the m element that we want to hide //d-none means display none
        } else {
            document.querySelector(`#m${m.id}`).classList.remove('d-none')
            count++
        }
    }
    featured_movie.innerHTML = count == 0 ? 'Nothing was found' : '' //? if then      : else
}
